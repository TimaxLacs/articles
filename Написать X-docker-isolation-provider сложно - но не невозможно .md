...
В один прекрасный день у нас на [платформе связей](https://github.com/linksplatform/) решили - было бы славно
портировать нашего бота в [Deep](https://github.com/deep-foundation). А для этого нужно было написать так [называемые провайдеры](https://github.com/deep-foundation?q=docker-isolation-provider&type=all&language=&sort=). 
...
Внимание конечно привлекает и

```
[Docker](https://www.docker.com/#build)
```

в названии, по [докер](https://www.docker.com/) файлу понятно, что образ просто запускается на машине
...
из возвращают данные). А я упорно отказывался делать DeepClient для Rust провайдера, потому что такого сгенерированного [Хасурой](https://hasura.io/) мрака я ещё не видывал. Да и в целом идея странная. 
...
Но не это самое грустно. Хуже всего было постоянно менять провайдер, чтобы он соотствествовал мнимому стандарту.
...
В коде ничего необычного. Выглядит это расширение синтаксиса как-то так:
    ...
    Вывод я пишу уже гораздо позже всей остальной статьи, так что уже и не помню конкретики. Ну в целом было весело, а ведь это самое главное.
